...
void main() {
  ...
  final baseInput = lines.join('\n');

  // Warmup with varied inputs to avoid cache hits.
  for (var i = 0; i < 200; i++) {
    KhmerUnicodeMapper.fixText('$baseInput#${i % 13}');
  }
  ...
  for (var i = 0; i < runs; i++) {
    checksum ^= KhmerUnicodeMapper.fixText('$baseInput#$i').length;
  }
  ...
  print('runs=$runs total_ms=${totalMs.toStringAsFixed(2)} avg_ms=${avgMs.toStringAsFixed(3)} checksum=$checksum');
}
